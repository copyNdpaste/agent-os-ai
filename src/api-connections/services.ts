...
export const API_SERVICES: ApiServiceDef[] = [
    ...
    },
    {
        ...
        name: 'PayPal (매출 분석)',
        icon: '💰',
        summary: '내 게임·서비스의 결제 거래를 분석. 매출 대시보드 + 새 결제 텔레그램 알림에 사용. Sandbox 와 Live 는 각각 별도 키 — Developer Dashboard 좌상단 토글로 발급해서 두 쌍 다 입력해두면 모드만 바꿔서 즉시 전환 가능.',
        helpUrl: 'https://developer.paypal.com/dashboard/applications',
        agentId: 'business',
        scopeHint: 'project-allowed',
        fields: [
            { key: 'PAYPAL_MODE', label: '활성 모드', type: 'select', options: ['sandbox', 'live'], help: '실제 분석에 사용할 모드. 두 쌍 다 입력해도, 한 번에 하나만 활성.' },
            { key: 'PAYPAL_SANDBOX_CLIENT_ID', label: '🧪 Sandbox · Client ID', type: 'password', help: 'Developer Dashboard 좌상단 Sandbox 토글 → Apps & Credentials → 본인 sandbox 앱.' },
            { key: 'PAYPAL_SANDBOX_CLIENT_SECRET', label: '🧪 Sandbox · Client Secret', type: 'password', help: '같은 sandbox 앱 화면에서 Secret 복사.' },
            { key: 'PAYPAL_LIVE_CLIENT_ID', label: '💵 Live · Client ID', type: 'password', help: 'Developer Dashboard 좌상단을 Live 로 토글 후 Live 앱에서 발급. 실제 결제 데이터.' },
            { key: 'PAYPAL_LIVE_CLIENT_SECRET', label: '💵 Live · Client Secret', type: 'password' },
