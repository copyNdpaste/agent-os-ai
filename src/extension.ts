...
/* Claude CLI 전환 후 모델 리스트는 3-tier 고정 — Opus 4.7 / Sonnet 4.6 / Haiku 4.5.
   ...
   같은 시그니처 유지. */
async function listInstalledModels(): Promise<{ id: string; backend: 'claude' }[]> {
  return [
...
class RevenueDashboardPanel {
    ...
    public static readonly viewType = 'agentOs.revenueDashboard';
    private readonly _panel: vscode.WebviewPanel;
    private _disposables: vscode.Disposable[] = [];
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            try {
                if (msg?.type === 'ready' || msg?.type === 'refresh') {
                    ...
                    vscode.commands.executeCommand('agentOs.apiConnections.open');
                }
            } catch (e: any) {
