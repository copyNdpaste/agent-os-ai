# 🧠 Agent OS — Brain-GitHub 동기화 아키텍처 레퍼런스
...
| 설정 키 | 용도 | 기본값 |
|---------|------|--------|
| `agentOs.localBrainPath` | 로컬 지식 폴더 절대 경로 | `""` (비면 `~/.agent-os-brain`) |
| `agentOs.secondBrainRepo` | 깃허브 저장소 URL | `""` |
| `agentOs.ollamaUrl` | AI 서버 주소 | `http://127.0.0.1:11434` |
| `agentOs.defaultModel` | AI 모델 이름 | `gemma4:e2b` |
| `agentOs.requestTimeout` | AI 응답 대기 시간(초) | `300` |
...
_getBrainDir()               // 로컬 지식 폴더 경로 반환 (미설정 시 ~/.agent-os-brain)
...
**Experiment Pack 주입 후 자동으로 호출되는 동기화 함수 (동기/Blocking)**
...
## 4. 데이터 흐름 (Experiment Pack 주입 ~ GitHub 동기화)

```
[외부 클라이언트] POST http://127.0.0.1:4825/api/brain-inject
...
# VS Code Settings → agentOs.localBrainPath 값 확인

# 2. 깃허브 URL이 저장되어 있는지?
# VS Code Settings → agentOs.secondBrainRepo 값 확인
...
# → agent-os-ai-{version}.vsix

# GitHub 릴리즈
gh release create v{version} agent-os-ai-{version}.vsix -t "Release v{version}"
